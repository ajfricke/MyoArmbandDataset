...
class Net(nn.Module):
    def __init__(self, number_of_class, ):
        ...
        self._batch_norm4 = nn.BatchNorm1d(64)
        self._prelu4 = nn.PReLU(64)
        self._dropout4 = nn.Dropout1d(.5)

        # Add LSTM
        self._lstm = nn.LSTM(16,16,1, batch_first=True)
    ...
    def forward(self, x):
        ...
        pool2 = pool2.flatten(start_dim=-2)
        intermediate, _ = self._lstm(pool2)
        lstm = self._dropout4(self._prelu4(self._batch_norm4(intermediate)))
        flatten_tensor = lstm.view(-1, 1024)
